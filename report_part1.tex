...
In order for both of these solutions to exist, we require that $\theta\in(0,1)$ because $\overline{v}=H(\overline{v}-\theta)$. In the case that $\overline{v}=1$,
...
Figure (\ref{test}) above demonstrates that as ????
...
Now we solve the partial differential equation to the left and the right of $V(\xi)=\theta$. The governing equation for $V(\xi)\neq\theta$ is 
\begin{equation}
\label{psiPDE}
...
Before proceeding with the analysis, we need to define the boundary conditions. Recall that the boundary condition $\lim_{x\to\pm\infty} \frac{\partial v(x,t)}{\partial x}=0$ is necessary to ensure a physically realistic solution. Thus, it must also be true that 
$$\lim_{x\to\pm\infty} \frac{\partial \big(V(\xi)+\epsilon\psi(\xi,t)\big)}{\partial x}=0 \text{ since $v(x,t)=V(\xi)+\epsilon\psi(\xi,t)$}$$
$$\lim_{x\to\pm\infty} \Big(\frac{dV}{d\xi}\frac{\partial \xi}{\partial x}+\epsilon\frac{\partial\psi}{\partial\xi}\frac{\partial \xi}{\partial x}\Big)=0$$
$$\lim_{\xi\to\pm\infty} \Big(\frac{dV}{d\xi}\Big)+\lim_{\xi\to\pm\infty} \Big(\epsilon\frac{\partial\psi}{\partial\xi}\Big)=0$$
We previously imposed the boundary condition that $\lim_{\xi\to\pm\infty} \Big(\frac{dV}{d\xi}\Big)=0$. It follows that 
\begin{equation}
\label{psiBC}
\lim_{\xi\to\pm\infty} \Big(\frac{\partial\psi}{\partial\xi}\Big)=0
\end{equation}
is the corresponding boundary condition for the solutions of equation \ref{psiPDE}.
